...
<form action="<?= site_url('products/update/' . $product['product_id']) ?>" method="post" class="bg-white p-4 rounded shadow-sm">
    <?= csrf_field() ?>
    ...
    <div class="mb-3">
        ...
        <input type="number" name="qty_in_stock" class="form-control"
               value="<?= old('qty_in_stock', $product['qty_in_stock']) ?>" min="0" required>
    </div>
